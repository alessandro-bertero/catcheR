...
TET = subset(meta_sh, meta_sh$knockdown == "TET")
CTR = subset(meta_sh, meta_sh$knockdown == "CTR")

ks_stat = data.frame()

for (c in unique(meta_sh$shRNA_label)) {
  for (analysis_type in analysis) {
    tmp_TET = TET[TET$shRNA_label == c, analysis_type]
    tmp_CTR = CTR[CTR$shRNA_label == c, analysis_type]
...
TET = subset(meta_cl, meta_cl$knockdown == "TET")
CTR = subset(meta_cl, meta_cl$knockdown == "CTR")

ks_stat = data.frame()

for (c in unique(meta_cl$clone)) {
  for (analysis_type in analysis) {
    tmp_TET = TET[TET$clone == c, analysis_type]
    tmp_CTR = CTR[CTR$clone == c, analysis_type]
...
ggplot(data=k_stat_clone, aes(x=k_sign_summary, y=log10(padj)*-1)) +
  ...
  ggtitle("ks test statistics per clone vs its control")+
  theme_minimal() +  # You can change the theme as needed
  labs(
    x = "KS statistic summary",
    y = "-Log10 adj p-value",
...
ggsave(filename = paste0(fname_prefix_dotplot, "_volcano_ks_stats_clone.pdf"),
       width = 6, height = 4)
...
ggsave(filename = paste0(fname_prefix_dotplot, "_volcano_ks_stats_clone_sig_FDR001.pdf"),
       width = 6, height = 4)
...
Rdata_filename = paste0(getwd(),"/FILES_PROCESSED_TO_UPLOAD/3D_iPS2seq10X_cds_monocle_processed.rds")
saveRDS(cds, Rdata_filename)

write.table(counts, file = paste0(getwd(),"/FILES_PROCESSED_TO_UPLOAD/3D_iPS2seq10X_cds_monocle_counts_prenorm.txt"), sep = "\t",
            row.names = T, col.names = T)
write.table(counts_n, file = paste0(getwd(),"/FILES_PROCESSED_TO_UPLOAD/3D_iPS2seq10X_cds_monocle_counts_norm.txt"), sep = "\t",
            row.names = T, col.names = T)
write.table(meta, file = paste0(getwd(),"/FILES_PROCESSED_TO_UPLOAD/3D_iPS2seq10X_monocle_cell_info.txt"), sep = "\t",
            row.names = T, col.names = T)
