...
dir.create(file.path("Output","monocle", format(Sys.Date(), "%y%m%d"), "cumulative"))
...
fname_prefix_dotplot <- file.path("Output","monocle", format(Sys.Date(), "%y%m%d"), "dotplots", 
                                                 format(Sys.Date(), "%y%m%d"))
fname_prefix_cumulative <- file.path("Output","monocle", format(Sys.Date(), "%y%m%d"), "cumulative", 
                                                 format(Sys.Date(), "%y%m%d"))
...
```{r}
load("Output/monocle/240314/R_objects/240314_cluster_composition_cardio_by_gene.RData")
...
ggplot(data=subset(ks.shRNA,ks.shRNA$shRNA!="B2M"&ks.shRNA$shRNA!="SCR"&ks.shRNA$shRNA!="empty"), aes(x=k_sign_summary_SCR, y=log10(padj_vs_SCR)*-1)) +
  geom_point(aes(colour = k_colour_SCR_all), size=2) +
  scale_colour_manual(values=c("#009E73","#D55E00","#CC79A7","grey"))+
  ggrepel::geom_text_repel(
...
ggplot(data=ks.stat_D12_clones, aes(x=k_sign_summary_SCR, y=log10(padj_vs_SCR)*-1)) +
  geom_point(aes(colour = k_colour_SCR), size=1.5) +scale_colour_okabeito(reverse=T)+
  ggrepel::geom_text_repel(
...
ggplot(data=subset(ks.clones,ks.clones$shRNA_gene!="B2M"&ks.clones$shRNA_gene!="empty"&ks.clones$shRNA_gene!="SCR"), aes(x=k_sign_summary_SCR, y=log10(padj_vs_SCR)*-1)) +
