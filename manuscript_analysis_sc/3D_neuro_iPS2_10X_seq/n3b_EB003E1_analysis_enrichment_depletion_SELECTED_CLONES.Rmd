...
dir.create(file.path("Output","E_D","specific_clones"))
dir.create(file.path("Output","E_D","specific_clones",format(Sys.Date(), "%y%m%d")))
dir.create(file.path("Output","E_D","specific_clones", format(Sys.Date(), "%y%m%d"), "R_objects"))
dir.create(file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "dotplots"))
dir.create(file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "dotplots","single"))
dir.create(file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "stat"))
dir.create(file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "csv"))
dir.create(file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "UMAP"))
dir.create(file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "mosaic"))
dir.create(file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "heatmap"))

fname_prefix_scratch="/scratch"
#set up forder path E_D
fname_prefix_R <- file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "R_objects", 
                               format(Sys.Date(), "%y%m%d"))
fname_prefix_dotplot <- file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "dotplots", 
                                                 format(Sys.Date(), "%y%m%d"))
fname_prefix_dotplot_single <- file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "dotplots", "single", 
                                                 format(Sys.Date(), "%y%m%d"))
fname_prefix_csv<-file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "csv", 
                               format(Sys.Date(), "%y%m%d"))
fname_prefix_stat<-file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "stat", 
                               format(Sys.Date(), "%y%m%d"))
fname_prefix_UMAP<-file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "UMAP", 
                               format(Sys.Date(), "%y%m%d"))
fname_prefix_mosaic<-file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "mosaic", 
                               format(Sys.Date(), "%y%m%d"))
fname_prefix_heatmap<-file.path("Output","E_D", "specific_clones",format(Sys.Date(), "%y%m%d"), "heatmap", 
                               format(Sys.Date(), "%y%m%d"))
...
```{r}
#####total Fisher test
sides=c("Side1", "Side2") # Sides to test
genes_to_test <- c(" CHD7", "SMAD2", "KMT2D","B2M","SCR") # Genes to test
cell_metadata_selection=cell_metadata%>%dplyr::filter(shRNA_gene%in%genes_to_test)
cell_metadata_selection=cell_metadata_selection%>%dplyr::filter(side%in%sides)
m_cl=cell_metadata_selection%>%dplyr::select(clone,KD)
...
counts=cell_metadata_selection %>%
...
totals=cell_metadata_selection%>%
  dplyr::group_by(KD)%>%
...
ggplot(data=ED_EB003E1_all_results, aes(x=log2(FC), y=(log2(percent_cl)))) +
  ...
  #xlim(-6,1)+
 # geom_vline(xintercept = c(fc_level, fc_level2))
  ggtitle("Clone enrichment depletion KD compared to CTR brain organoids")+
  theme_minimal() +  # You can change the theme as needed
  labs(
    x = "Log2 of clone fraction in KD vs CTR",
    y = "Log2 of clone fraction",
...
ggsave(filename = paste0(fname_prefix_dotplot, "_EB003E1_clone_enrichment_depletion_CTR_TET.pdf"),
       width = 8, height = 5)
```
...
```{r}
cell_metadata_s2=subset(cell_metadata_selection,cell_metadata_selection$side=="Side2")
