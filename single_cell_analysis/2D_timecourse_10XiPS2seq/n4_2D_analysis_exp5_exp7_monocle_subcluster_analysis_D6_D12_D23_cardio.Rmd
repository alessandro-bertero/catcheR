...
dir.create(file.path("Output","CARDIO","monocle"))#create forders cardio MONOCLE
...
stat_filename = paste0(fname_prefix_stat, "_Freq_cells_by_shRNA_cardio.csv")
...
ggplot(data=D12_result_fisher_shRNA, aes(x=log2(percent_over_SCR_D12), y=(log10(q_value_SCR)*-1))) +
  geom_point(aes(colour = factor(cell_type_short,levels=c(ctD12$order_cell_types)), size=1.5)) +
  scale_color_manual(name="cell types",values=ctD12$okabe_clust)+
  #scale_colour_manual(values = pal)+
  #geom_text(aes(label=to_show_2vsother), size=4, hjust=1.2)+
  ggrepel::geom_text_repel(
    data = subset(D12_result_fisher_shRNA,D12_result_fisher_shRNA$sig_SCR==T),
    aes(label = shRNA),
    size = 3,
    box.padding = unit(0.35, "lines"),
    point.padding = unit(0.3, "lines")
...
ggplot(data=D23_result_fisher_shRNA, aes(x=log2(percent_over_SCR_D23), y=(log10(q_value_SCR)*-1))) +
  geom_point(aes(colour = factor(cell_type_short,levels=c(ctD23$order_cell_types)), size=1.5)) +
  scale_color_manual(name="cell types",values=ctD23$okabe_clust)+
  #scale_colour_manual(values = pal)+
  #geom_text(aes(label=to_show_2vsother), size=4, hjust=1.2)+
  ggrepel::geom_text_repel(
    data = subset(D23_result_fisher_shRNA,D23_result_fisher_shRNA$sig_SCR==T),
    aes(label = shRNA),
    size = 3,
    box.padding = unit(0.35, "lines"),
    point.padding = unit(0.3, "lines")
...
ggsave(filename = paste0(fname_prefix_UMAP, "_", "UMAP_clustered.pdf"),
     width = 15, height = 4)
...
ggsave(filename = paste0(fname_prefix_UMAP, "_", "UMAP_clustered_compact.pdf"),
     width = 5, height = 4)
...
ggplot(data=D6_result_fisher_clone%>%filter(gene=="SCR"|gene=="B2M"|gene=="empty"|gene=="GATA4"), aes(x = cell_type_short, y = percent_clone_D6, fill=factor(shRNA_label,levels=c(order_shGATA)))) + geom_bar(stat = "identity") +
  ...
  facet_wrap(~factor(clone,levels=c(data_clones_pal$clone)), ncol=9)
...
ggplot(data=D6_result_fisher_clone%>%filter(gene=="SCR"|gene=="B2M"|gene=="empty"|gene=="NKX2.5"), aes(x = cell_type_short, y = percent_clone_D6, fill=factor(shRNA_label,levels=c(order_shNKX)))) + geom_bar(stat = "identity") +
  ...
  facet_wrap(~factor(clone,levels=c(data_clones_pal$clone)), ncol=4)
